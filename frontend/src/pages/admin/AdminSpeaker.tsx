...
export default function AdminSpeaker() {
    ...
    const fetchApplications = async () => {
        try {
            ...
            const response = await fetch(`${API_URL}/api/speaker-application/applications`, {
                headers: {
                            ...
                            {applications.map((app) => (
                                <tr key={app.id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 hidden lg:table-cell">
                                        {app.linkedinLink ? (
                                            <a href={app.linkedinLink} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-sm truncate block max-w-50">
                                                {app.linkedinLink}
                                            </a>
                                        ) : (
                                            '-'
